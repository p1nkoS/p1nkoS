...
export default function Header({ onOpenForm }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      >
        <div className="max-w-[1600px] mx-auto px-6 py-2.5 flex items-center justify-between text-white text-[12px]">
        ...
        <nav className={`mt-3 rounded-full transition-all duration-500 ${scrolled ? "bg-white shadow-lg" : "bg-white/95 backdrop-blur-md shadow-md"}`}>
          <div className="flex items-center justify-between px-4 md:px-6 py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={onOpenForm}
                className="btn-shimmer rounded-full bg-[#FF5722] hover:bg-[#e64a17] text-white shadow-md hidden md:inline-flex px-6 py-5 font-semibold text-[13px]"
              >
                Залишити заявку
              </Button>
              <Button
                onClick={onOpenForm}
                className="md:hidden inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#FF5722] hover:bg-[#e64a17] text-white shadow-md p-0"
