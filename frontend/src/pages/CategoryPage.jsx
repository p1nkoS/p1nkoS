...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    setChambers([]);
    setSoundMin(0);
    if (t === "alu") setSelectedMaterial("Алюміній");
    else if (t === "pvc") setSelectedMaterial("Металопластик");
    else setSelectedMaterial("all");
  ...
  const FilterPanel = () => (
    <div className="space-y-7">
      ...
      {/* Material - only for sliding */}
      {slug === "sliding" && (
        <div>
          <label className="block text-xs uppercase tracking-wider text-[#888] mb-3 font-semibold">Тип системи</label>
          <div className="flex flex-wrap gap-2">
            {["all", "Металопластик", "Алюміній"].map((m) => (
              <button
                key={m}
      ...
      <div>
        ...
        <Slider
          ...
          className="my-3"
        />
        <div className="flex items-center justify-between text-sm text-[#1a1a1a] mt-3">
          <span className="px-3 py-1 bg-[#f5f1ec] rounded-lg font-medium tabular-nums">{priceRange[0].toLocaleString()} ₴</span>
          <span className="px-3 py-1 bg-[#f5f1ec] rounded-lg font-medium tabular-nums">{priceRange[1].toLocaleString()} ₴</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="text-sm text-[#1a1a1a] font-medium mt-3 tabular-nums">від {soundMin} дБ</div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#f5f1ec]">
      ...
      <section className="relative pt-32 pb-12 lg:pt-40 lg:pb-16 overflow-hidden">
        ...
        <div className="relative z-[3] max-w-[1600px] mx-auto px-6 text-white">
      ...
      <section className="py-10 lg:py-14">
        <div className="max-w-[1600px] mx-auto px-6 grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-9">
            <div className="hidden lg:flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <p className="text-[#5a5a5a] text-sm mr-4">Знайдено <span className="font-bold text-[#1a1a1a]">{filtered.length}</span> продуктів</p>
                {slug === "sliding" && (
                  <div className="flex bg-white rounded-full p-1 border border-[#e6dfd5]">
                    {["all", "Металопластик", "Алюміній"].map((m) => (
                      <button
                        key={m}
                        onClick={() => setSelectedMaterial(m)}
              ...
              {(brands.length || countries.length || chambers.length || soundMin > 0 || selectedMaterial !== "all") ? (
              ...
              <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {filtered.map((p) => (
                  <Link
                    key={p.slug}
                    to={`/product/${p.slug}`}
                    className="bg-white rounded-[24px] overflow-hidden card-hover relative group reveal border border-[#f0e9e0]"
                  >
                    {p.badge && <span className="ribbon-tag">{p.badge}</span>}
                    <div className="h-60 overflow-hidden bg-[#f5f1ec]">
                      <SmartImage 
                        src={p.image} 
                        alt={p.name} 
                        width={600} 
